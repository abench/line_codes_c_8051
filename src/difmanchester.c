...
void display_welcome_screen()
{
  LCD_CMD = (CLEAR_CMD);	
  delay16(300);
  string_to_hd(0,0,"DIF.");
  string_to_hd(0,1,"MANCHESTER");
...
void encode_dif_manchester()
{
 ...
 // unsigned int z = ONE_MINUS_LEVEL;
   ...
   switch (inp_sequence[0])
   {               
    case '1': 
               line_sequence[0]=ONE_PLUS_LEVEL;
               line_sequence[1]=ONE_MINUS_LEVEL;
               break;              
    case '0':
               line_sequence[0]=ONE_MINUS_LEVEL;
               line_sequence[1]=ONE_PLUS_LEVEL;
               break;
   };  
 ...
 for (i=1;i<SEQUENCE_LEN;i++)
 {
   ...
   {               
    case '1': 
               line_sequence[2*i]=line_sequence[2*i-1];

               break;              
    case '0':
               if (line_sequence[2*i-1]=ONE_MINUS_LEVEL) line_sequence[2*i] = ONE_PLUS_LEVEL; else line_sequence[2*i+1] = ONE_MINUS_LEVEL;
               break;             
   };  
   if (line_sequence[2*i]=ONE_MINUS_LEVEL) line_sequence[2*i+1] = ONE_PLUS_LEVEL; else line_sequence[2*i+1] = ONE_MINUS_LEVEL; 
 }
